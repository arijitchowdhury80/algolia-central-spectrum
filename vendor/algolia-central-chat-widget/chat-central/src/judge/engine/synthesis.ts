...
type JudgeScore = { temperament: Judgment['temperament']; score: number };

/** Trimmed-skeptic-weighted consensus: removes the most generous non-skeptic
 *  when the panel has 3+ judges, then weights the Skeptic by `skepticWeight`. */
function trimmedSkepticWeighted(onFinal: JudgeScore[], skepticWeight: number): number {
...
function consensusScore(
  ...
  const onFinal: JudgeScore[] = judgments.map((j) => ({
  ...
  switch (cfg.rule) {
    ...
    case 'trimmed-skeptic-weighted':
      return trimmedSkepticWeighted(onFinal, cfg.skepticWeight ?? 1.5);
  }
}
 ...
 *
 * The rationale defaults to the gate explanation. Callers may override it by
 * spreading `{ ...result, rationale: llmAuthored }`. This function is PURE —
...
const DEFAULT_GATE_CLEAN_THRESHOLD = 1 / 3;

interface AggregateRoundsConfig {
  rubric: Rubric;
  synthesisCfg: SynthesisConfig;
  gateCfg: HardGateConfig;
...
/** Compute per-dimension score means across all rounds and all judges. */
function computeDimensionMeans(
...
/** Compute per-judge composite scores (weightedScore → 0-10) averaged across rounds. */
function computeJudgeComposites(
  perRoundJudgments: readonly (readonly Judgment[])[],
  rubric: Rubric,
): Array<{ judgeId: string; temperament: Judgment['temperament']; composite: number }> {
  ...
  for (const js of perRoundJudgments) {
    for (const j of js) {
      const cur = compSums.get(j.judgeId) ?? { temperament: j.temperament, total: 0, n: 0 };
      cur.total += toFinalScale(j.weightedScore, rubric);
      cur.n += 1;
      compSums.set(j.judgeId, cur);
    }
  }
  return [...compSums.entries()].map(([judgeId, { temperament, total, n }]) => ({
    judgeId,
    temperament,
...
export function aggregateRounds(
  perRoundJudgments: readonly (readonly Judgment[])[],
  cfg: AggregateRoundsConfig,
): RoundAggregate {
  const {
    rubric,
    synthesisCfg,
    gateCfg,
    tripThreshold = DEFAULT_GATE_VOTE_THRESHOLD,
    cleanThreshold = DEFAULT_GATE_CLEAN_THRESHOLD,
  } = cfg;
  const rounds = perRoundJudgments.length;
  const perRoundPreGate = perRoundJudgments.map((js) => consensusScore(js, rubric, synthesisCfg));
  ...
  const maxRecurrence = claimGate.clusters.reduce((m, c) => Math.max(m, c.recurrenceFraction), 0);

  const dimensionMeans = computeDimensionMeans(perRoundJudgments);
  const judgeComposites = computeJudgeComposites(perRoundJudgments, rubric);
  ...
  const finalScore = gateTripped ? Math.min(meanPreGateScore, gateCfg.cap) : meanPreGateScore;
