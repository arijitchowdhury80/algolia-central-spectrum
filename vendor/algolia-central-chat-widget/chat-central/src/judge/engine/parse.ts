import type { DimensionScore, GroundingViolation, Rubric } from './types.js';
...
/** Scan forward from `i` (which is a `"` character) and return the index of the
 *  matching closing `"`, honouring backslash escapes. Returns `raw.length - 1`
 *  if no closing quote is found (malformed string — caller keeps scanning). */
function advancePastString(raw: string, i: number): number {
  let escaped = false;
...
export function parseJudgeOutput(
  ...
  const groundingViolations: GroundingViolation[] = (obj.groundingViolations ?? []).map((v) => ({
