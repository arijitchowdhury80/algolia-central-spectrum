 ...
 *        --pagebase https://spectrum.adobe.com/page \
 *        --urlbase https://github.com/adobe/spectrum-design-data/blob/main/docs/s2-docs [--dry]
 *
 * --pagebase  preferred citation target: joined with each file's frontmatter
 *             `source_url` slug to form the real docs page.
 * --urlbase   fallback for files with no slug. Use the repo's /blob/ url so a
 *             click opens a RENDERED page; a raw.githubusercontent base makes
 *             every citation a plaintext download (see citationUrl below).
 */
...
/**
 * THE CITATION URL — what a user's click actually opens. NOT the fetch url.
 *
 * 2026-07-28: the live SpectrumDesignDocs records cited
 * `raw.githubusercontent.com/.../x.md`, so clicking a source in the chat UI
 * dumped plaintext markdown at the user. On a demo whose claim is "every answer
 * cites its source", a citation must open something a human can read.
 *
 * Order of preference:
 *   1. `--pagebase` + the file's frontmatter `source_url` slug — the real docs
 *      page. Frontmatter `source_url` often names an INTERNAL host (Adobe's
 *      files say `s2.spectrum.corp.adobe.com`, which does not resolve publicly),
 *      so only its slug is reused, against the public base the caller passes.
 *   2. `--urlbase` + the relative path. Pass a repo BLOB base
 *      (github.com/<org>/<repo>/blob/<ref>/<subdir>) — GitHub renders that as a
 *      readable page. Do NOT pass a raw.githubusercontent base: it is a
 *      plaintext download, which is the bug this exists to prevent.
 *
 * Verifying that a `--pagebase` page really exists is the caller's job — see
 * `repair_citation_urls.mjs`, which checks for a <title> because these doc sites
 * are SPAs that answer 200 with an empty shell for unknown slugs.
 */
function citationUrl(fm, rel) {
  const slug = (fm.source_url || '').match(/\/page\/([^/]+)\/?/)?.[1] ?? null;
  if (A.pagebase && slug) return `${String(A.pagebase).replace(/\/$/, '')}/${slug}/`;
  if (A.urlbase) {
    if (String(A.urlbase).includes('raw.githubusercontent.com')) {
      console.warn(`  WARNING: --urlbase is a raw.githubusercontent base — citations will open a plaintext .md download, not a page. Use the /blob/ url instead.`);
    }
    return `${String(A.urlbase).replace(/\/$/, '')}/${rel}`;
  }
  return undefined;
}
