...
import { SourcePanel, SourceRow, useSourceGroups } from './SourcePills';
...
import { useCostRecording } from '../hooks/useCostRecording';
...
function SegmentView({
  ...
  useCostRecording(turnId, segment, question, verdict);
  // Accordion state lives here, not inside the pill row, because the row and its
  // dropdown are rendered on either side of the cost/chip line below.
  const sourceGroups = useSourceGroups(segment.sources);
    ...
    <div className="flex flex-col gap-3 rounded-ac-xl bg-ac-surface p-6 shadow-ac-2" aria-busy={busy}>
      ...
      {emptyResult && (
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-ac-sm text-ac-text-muted">
          {/* Empty-but-no-error = the Agent Studio cached-empty flake. A same-query
              retry can't escape a poisoned semantic cache key, so we nudge a
              rephrase rather than imply retry will fix it. */}
          <span>No answer came back — try rephrasing your question.</span>
          <button
            type="button"
            onClick={onRetry}
      ...
      {segment.status === 'success' && (segment.sources.length > 0 || canJudge) && (
        // The source pill row shares its line with the cost badge + grounding
        // chip; the open category's dropdown is rendered AFTER that line so
        // expanding a category doesn't drag the badges down with it.
        <>
          <div className="flex items-end justify-between gap-4">
            <div className="min-w-0 flex-1">
              <SourceRow src={sourceGroups} />
            </div>
            <div className="flex shrink-0 items-center gap-2">
              <CostBadge turnId={turnId} agent={segment.agent} />
              {canJudge && (
                <ConfidenceChip
                  verdict={verdict}
