...
export interface AppHeaderProps {
  ...
  onReset: () => void;
  /** Open the session-wide cost page (spike plan §6). */
  onOpenCost: () => void;
}
 ...
 *  home" affordance. */
export function AppHeader({ onReset, onOpenCost }: AppHeaderProps) {
  return (
    <header
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <button
          type="button"
          onClick={onOpenCost}
          title="Session cost — cumulative tokens + spend across this conversation"
          className="rounded-ac-full border border-ac-border px-3 py-1 text-ac-xs font-ac-medium text-ac-text-secondary transition-colors duration-ac-fast ease-ac-ease hover:border-ac-accent hover:bg-ac-surface-hover"
        >
          Cost
        </button>
