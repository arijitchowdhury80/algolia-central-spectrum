...
- **`scripts/agents/agentConfig.mjs` declares `MAIN_MODEL = 'gemini-2.5-flash'`, but every live agent runs `medium`.** Running the agent build script would change the live model. Reconcile the constant before using it.
