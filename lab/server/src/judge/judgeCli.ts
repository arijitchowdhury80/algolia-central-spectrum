 ...
 *
 * PORTED VERBATIM from AC2 lab/server/src/judge/judgeCli.ts (imports only
 * ../activeJudgeLlm.js and ./liveJudge.js — both ported alongside this file).
 * An agent or a human pipes one request JSON in and gets the single-dimension
 * (Usefulness) + corroboration-gate verdict JSON back on stdout (Phase 2
 * rebuild). Same core, provider, and request/response shapes as the HTTP
 * service — just a process instead of a socket.
 *
 ...
 *     { "rounds", "panels":[ { panelId, dims:{usefulness},
 *       synthesizedScore, composite, gateTripped, borderline,
 ...
 *     npm run judge:cli -- --file path/to/request.json
 */
