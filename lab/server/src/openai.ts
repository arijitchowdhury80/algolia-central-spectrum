 ...
 *
 * PORTED VERBATIM from AC2 lab/server/src/openai.ts (self-contained, only
 * imports @lab/judge's types). This is the only seam the AI Judge needs: it
 * injects this `openaiComplete` (matching @lab/judge's LlmComplete type) and
 ...
 * models. Some reasoning models reject a non-default `temperature` — the
 * temperature-drop retry below handles that defensively.
 */
...
interface OpenAIResponse {
  ...
  /** Exact token accounting — read but previously discarded. Cost tracking §6. */
  usage?: OpenAIUsage;
...
export interface OpenAIClientConfig {
  ...
  /**
   * OpenAI-compatible API base (the part ending in `/v1`). Default: OpenAI's
   * own `https://api.openai.com/v1`. Set this to point at any OpenAI-compatible
   * endpoint — e.g. Algolia's enablers inference server
   * (`https://inference.api.enablers.algolia.net/v1`). `/chat/completions` is
...
export function makeOpenAIComplete(cfg: OpenAIClientConfig): LlmComplete {
  ...
  async function call(body: RequestBody, opts?: LlmCompleteOptions): Promise<string> {
  ...
  return async function openaiComplete(
    ...
    try {
      return await call(body, opts);
    } catch (e) {
      const err = e as Error & { bodyText?: string };
      const msg = err.bodyText ?? err.message ?? "";
      // Reasoning models reject non-default temperature -> drop & retry.
