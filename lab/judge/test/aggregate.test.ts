...
import type { Rubric } from "../src/index.js";

/**
 * The generic weighted-aggregation MATH is rubric-agnostic (aggregate.ts is
 * unchanged by the Phase 2 rebuild — only ALGOLIA_ANSWER_RUBRIC shrank to one
 * dimension). Exercise it here against a synthetic 4-dimension rubric so this
 * suite still tests the multi-dimension weighting behaviour for real.
 */
const FOUR_DIM_RUBRIC: Rubric = {
  name: "test 4-dimension rubric",
  min: 1,
  max: 10,
  dimensions: [
    { id: "grounding", label: "Grounding", description: "", weight: 1 },
    { id: "coverage", label: "Coverage", description: "", weight: 1 },
    { id: "depth", label: "Depth", description: "", weight: 1 },
    { id: "relevance", label: "Relevance", description: "", weight: 1 },
  ],
};

function flatScores(value: number) {
  return FOUR_DIM_RUBRIC.dimensions.map((d) => ({
    dimensionId: d.id,
    score: value,
    rationale: "test",
  }));
...
describe("weightedAggregate", () => {
  it("returns the flat value when every dimension scores the same", () => {
    ...
    const agg = weightedAggregate(scores, FOUR_DIM_RUBRIC);
    expect(agg).toBe(8);
  });
  ...
  it("composite is the equal-weighted mean of the 4 dimensions (grounding x1)", () => {
    ...
    const composite = weightedAggregate(dims, FOUR_DIM_RUBRIC);
  ...
  it("weights all 4 dimensions equally: any single low dimension pulls the mean the same", () => {
    ...
    const aggAlt = weightedAggregate(scoresAlt, FOUR_DIM_RUBRIC);
  ...
  it("supports optional dimensions via the applicability filter", () => {
    // Verify the mechanism on a synthetic rubric with one optional dimension.
    const withOptional = {
      ...FOUR_DIM_RUBRIC,
      dimensions: [
        ...FOUR_DIM_RUBRIC.dimensions,
  ...
  it("the real ALGOLIA_ANSWER_RUBRIC (single usefulness dimension) aggregates to that one score", () => {
    const scores = [{ dimensionId: "usefulness", score: 7, rationale: "test" }];
    expect(weightedAggregate(scores, ALGOLIA_ANSWER_RUBRIC)).toBe(7);
  });
});
