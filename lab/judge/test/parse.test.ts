...
describe("parseJudgeOutput traceable-excerpt fields (Phase 2)", () => {
  it("carries sourceId/excerpt through when present", () => {
    const raw = JSON.stringify({
      dimensionScores: [],
      groundingViolations: [
        {
    ...
    const out = parseJudgeOutput(raw, rubric);
    expect(out.groundingViolations[0].sourceId).toBe("S8");
    expect(out.groundingViolations[0].excerpt).toBe("the exact quoted text");
    // excerptVerified is NOT set by parse — that's excerptCheck.ts's job.
    expect(out.groundingViolations[0].excerptVerified).toBeUndefined();
  });

  it("legacy output without sourceId/excerpt yields empty strings (back-compat)", () => {
    const raw = JSON.stringify({
      dimensionScores: [],
      groundingViolations: [{ claim: "x", reason: "y", certainty: 0.9, kind: "contradicted" }],
    });
    const out = parseJudgeOutput(raw, rubric);
    expect(out.groundingViolations[0].sourceId).toBe("");
    expect(out.groundingViolations[0].excerpt).toBe("");
  });
});
