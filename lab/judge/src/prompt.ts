...
/**
 * The sentence enforcing the Usefulness/grounding orthogonality that is the
 * core of the Phase 2 redesign (spec §2.5): a judge must not let grounding
 * doubts bleed into the Usefulness number — that separation is handled by
 * groundingViolations + the gate, not this score.
 */
const USEFULNESS_GROUNDING_SEPARATION =
  "Score Usefulness on completeness and concrete specificity ONLY. Grounding " +
  "is judged separately via groundingViolations and must NOT change the " +
  "Usefulness number — a fluent answer with an unsupported claim is still " +
  "Useful if it is complete and specific; the gate, not this score, handles " +
...
function renderRubric(rubric: Rubric, dims: ReturnType<typeof applicableDimensions>): string {
  ...
  return (
    `RUBRIC "${rubric.name}" — score each dimension on an integer ${rubric.min}-${rubric.max} scale:\n${lines.join("\n")}\n\n` +
    USEFULNESS_GROUNDING_SEPARATION
  );
}
...
export function renderExpectedCoverage(artifact: Artifact): string {
  ...
  return (
    "EXPECTED COVERAGE (the answer should address each; feed the Usefulness completeness judgement):\n" +
    lines.join("\n")
  );
}
...
export function buildSynthesisPrompt(
  ...
  return [
    ...
    `already been computed by the panel math: ${computedFinalScore.toFixed(2)}/10` +
      (gateTripped
        ? " (a grounding hard-gate was TRIPPED because ≥2 of the 3 judges independently flagged the same unsupported claim — explain that the cap was applied because of that corroborated claim)."
        : "."),
    "",
    `ARTIFACT TYPE: ${artifact.type}`,
    "",
