...
export interface RawJudgeOutput {
  ...
  groundingViolations?: {
    ...
    confidence?: number;
    kind?: string;
    /** Phase 2 traceable-excerpt fields (spec §2.2/§2.3). */
    sourceId?: string;
    excerpt?: string;
  }[];
  summary?: string;
...
export function parseJudgeOutput(
  ...
  const groundingViolations: GroundingViolation[] = (obj.groundingViolations ?? []).map(
    (v) => ({
      ...
      // Phase 2 traceable-excerpt fields. excerptVerified is NOT set here — the
      // post-validator (excerptCheck.ts) sets it after parse, because parse.ts
      // has no access to the Source texts.
      sourceId: v.sourceId != null ? String(v.sourceId) : "",
      excerpt: v.excerpt != null ? String(v.excerpt) : "",
    }),
  );
