...
Skeptic / Referee / Advocate, all at temperature 0 — over a single **Usefulness**
dimension (Phase 2 rebuild, 2026-07-19: the earlier 4-dimension rubric collapsed
to one — grounding is no longer scored, only gated), applies a **corroboration
gate** (a claim only caps the score when ≥2 of the 3 distinct judges independently
flag it — a solo flag surfaces as `borderline` and never caps), and (over multiple
rounds) pools that gate across rounds to kill run-to-run flicker.
...
4. Reconciles the panel into one **pre-gate consensus** score, then applies the
   **corroboration gate** (`src/synthesis.ts`, `src/gate.ts`, Phase 2 rebuild):
   a `contradicted` flag at ≥0.7 certainty is gate-eligible, flags are clustered
   by claim similarity across judges, and the score is only capped when a
   cluster is flagged by **≥2 of the 3 distinct judges** — a solo flag surfaces
   as `borderline` and never caps. (`src/claimGate.ts`'s older round-recurrence
   gate is exported for back-compat but is no longer on the default path — temp
   0 makes same-judge rounds redundant; cross-judge corroboration is the real
   signal.)
5. Optionally runs **N rounds**; `aggregateRounds` pools the corroboration gate
   across all of them for a stable voted verdict — the zero-flicker guarantee.
...
console.log(result.aggregate.dimensionMeans); // { usefulness }
console.log(result.aggregate.gateTripped);    // corroboration gate (>= 2 distinct judges)
console.log(result.aggregate.borderline);     // solo flag exists, but didn't cap
console.log(result.aggregate.corroboratedClusters); // clusters that capped the score
console.log(result.aggregate.soloFlags);            // clusters flagged by exactly 1 judge
```

`DEFAULT_JUDGE_CONFIG` bundles the single-dimension `usefulness` rubric, the 3
personas, the gate (`cap 3`, `verifiedConfidence 0.7`, `corroborationThreshold 2`,
`claimSimThreshold 0.5` — ANY judge can flag, corroboration decides what caps),
and `mean` synthesis. Pass your own `JudgeConfig` to customize the rubric,
personas, gate, or vote threshold.
...
```jsonc
{
  ...
  "panels": [
    {
      "panelId": "P1",
      "dims": { "usefulness": 7.2 },                                        // the ONE scored dim, 1–10
      "dimensions": [ { "id": "usefulness", "label": "Usefulness", "score": 7.2 } ],
      "synthesizedScore": 7.2, "composite": 7.2,   // the Confidence composite (0–10)
      "preGateScore": 7.2,
      "gateTripped": false,                        // corroboration gate (>= 2 distinct judges)
      "borderline": false,                          // solo flag exists, but didn't cap
      "corroboratedClusters": [],                   // clusters that capped the score (empty here)
      "soloFlags": [],                              // clusters flagged by exactly 1 judge
      "flaggedClaims": [ { "claim": "...", "reason": "...", "certainty": 0.4, "judgeIds": ["skeptic"], "sourceId": "S1", "excerpt": "...", "excerptVerified": true } ],
...
interface RoundAggregate {
  ...
  gateTripped: boolean;          // corroboration gate — tripped iff >= 2 distinct judges corroborate a claim
  borderline: boolean;           // a solo flag exists, but didn't cap — visible, non-capping signal
  corroboratedClusters?: CorroboratedCluster[];  // clusters that capped the score (pooled across rounds)
  soloFlags?: CorroboratedCluster[];             // clusters flagged by exactly 1 distinct judge
  finalScore: number;            // = composite: meanPreGateScore capped iff gateTripped
  dimensionMeans: Record<string, number>;   // { usefulness } — Phase 2 rebuild, one scored dim
